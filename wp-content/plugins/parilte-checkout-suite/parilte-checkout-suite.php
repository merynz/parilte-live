...
add_action('wp_enqueue_scripts', function () {
    ...
    wp_add_inline_script('jquery', "jQuery(function($){
      ...
      // Mobile offcanvas: show full category tree, remove duplicate account/cart items
      var \$panel = $('.ct-panel, .ct-offcanvas-panel').first();
      if (\$panel.length && \$treeSrc.length) {
        var \$treeClone = \$treeSrc.find('.parilte-cat-tree').first().clone(true);
        if (\$treeClone.length && !\$panel.find('.parilte-cat-tree').length) {
          \$panel.find('.ct-panel-content').prepend(
            $('<div class=\"parilte-offcanvas-cats\"></div>').append(\$treeClone)
          );
        }
        \$panel.find('a').filter(function(){
          var href = (\$(this).attr('href') || '').toLowerCase();
          var txt = \$.trim(\$(this).text()).toLowerCase();
          return href.indexOf('/hesabim') > -1 || href.indexOf('/sepet') > -1 || txt === 'hesabım' || txt === 'sepet' || txt === 'kategoriler' || txt === 'kategori';
        }).closest('li').remove();
      }
    ...
    <main id="primary" class="site-main parilte-front">
      ...
      <section class="parilte-mag-cats">
        <div class="parilte-container">
          ...
          <div class="parilte-mag-cats-grid">
      ...
      <section class="parilte-mag-lookbook">
        <div class="parilte-container parilte-mag-lookbook-grid">
          <div class="parilte-mag-lookbook-copy">
          ...
          <div class="parilte-mag-lookbook-media" style="background-image:url('<?php echo esc_url($assets['look']); ?>');"></div>
        </div>
      </section>
...
function parilte_cs_footer_links() {
    ...
    <div class="parilte-legal-footer">
      <div class="parilte-container">
        ...
        <div class="parilte-legal-contact" aria-label="İletişim">
          <span>Instagram: <a href="https://www.instagram.com/butik_parilte_/" target="_blank" rel="noopener">@butik_parilte_</a></span>
          <span>WhatsApp: <a href="https://wa.me/905394353913" target="_blank" rel="noopener">0539 435 39 13</a></span>
          <span>Mail: <a href="mailto:g-zemkoroglu-772011@hotmail.com">g-zemkoroglu-772011@hotmail.com</a></span>
          <span>Adres: Parılte Butik, Rüstempaşa Mah. Çeşme Sk. No:17/D, Yalova/Merkez</span>
        </div>
...
add_action('wp_enqueue_scripts', function () {
    ...
    .parilte-mag-hero{padding:clamp(28px,6vw,64px) 0;background:linear-gradient(180deg,var(--parilte-surface),var(--parilte-cream));border-bottom:1px solid var(--parilte-border)}
    ...
    .parilte-mag-hero-media{width:100%;aspect-ratio:4/3;background-size:cover;background-position:center;border-radius:clamp(16px,4vw,28px);box-shadow:var(--parilte-shadow)}
    .parilte-mag-story{padding:clamp(24px,6vw,56px) 0}
    .parilte-mag-story-grid{display:grid;gap:clamp(14px,3vw,24px);grid-template-columns:1fr}
    ...
    .parilte-mag-cats{padding:clamp(24px,6vw,56px) 0}
    ...
    .parilte-mag-cat span{opacity:.7}
    .parilte-mag-lookbook{padding:clamp(24px,6vw,56px) 0}
    ...
    .parilte-mag-journal{padding:clamp(20px,5vw,52px) 0}
